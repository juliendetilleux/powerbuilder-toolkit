...
SELECT
   monthnum.num accounting_period,
	...
	where MONTH(invoice_date) = monthnum.num AND
		...
		customer_group = adresgroup.agcode and
  ...
  WHERE adresgroup.agcode IN (select distinct customer_group
										  from yq_sales
									    where if :as_customer_group = '*' then '*' else customer_group endif = if :as_customer_group = '*' then '*' else :a
```
