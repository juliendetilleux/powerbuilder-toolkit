...
```sql
  SELECT truckhead.thcode,   
         ...
         (select choices.chname from choices where choices.chtype = 'PAYM' and choices.chcode = invoicehead.ihpaymnt) as paym,
			...
			truckhead.thdriver,
         shiphead.shprseq,
		(select list(string( cast(shippack.spqty as numeric(6,0)) ) + ' ' + packings.pkcode)
				from shippack, packings
				where shippack.spcode = shiphead.shcode AND
						packings.pkcode = shippack.sppackid AND
						packings.pktype = '1') as itempack ,
	( select first turnhead.thdesc 
		from salhead, turnhead 
		where shipline.slsalorder = salhead.shcode and
			turnhead.thid = salhead.shturnid ) as turn 
    FROM salhead,   
         shiphead,   
         shipline,   
         truckhead,   
