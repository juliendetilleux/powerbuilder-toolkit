# zmod_actions_services_line_subprint
...
```sql
  SELECT adrsactions.aacode,   
         ...
         isnull(adrsactions.aacomment,'') as aacomment,   
         adrsactions.aadesc,   
         ...
         ( select fldesc from fileline where fileline.flcode = adrsactions.aafileref and fileline.flline = adrsactions.aafileline ),
			...
			isnull((select items.itum from items where itcode = adrsactions.aaitem),'') as itum
    FROM adrsact
```
