# d_clot_sal_profit_cust_multi_print
...
```sql
  SELECT isnull((SELECT sum(closal.cdqty)
						...
						WHERE closal.cditem = items.itcode
							...
							AND closal.cdperiod <= :Sel_period2 AND
(:MultiCo = '*' OR :MultiCo = coalesce(closal.cdmccode ,'##########')) /*jm012 */
			...
			isnull((SELECT sum(clot.cdqty)
								...
								WHERE clot.cditem = items.itcode
									...
									AND clot.cdperiod <= :Sel_period2 AND
(:MultiCo = '*' OR :MultiCo = coalesce(clot.cdmccode ,'##########')) /*jm012 */
			...
			isnull((SELECT sum(cloext.ciqty)
							...
							WHERE cloext.ciitem = items.itcode
								...
								AND cloext.ciperiod <= :Sel_period2 AND
(:MultiCo = '*' OR :MultiCo = coalesce(cloext.cimccode,'##########')) /*jm012 */
         ...
         sum(sales.cdval)  sale_val,  
			...
			if sum(if sales.cdqty is null or sales.cdqty = 0 or vclosal =
```
