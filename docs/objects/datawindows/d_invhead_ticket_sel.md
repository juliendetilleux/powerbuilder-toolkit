...
```sql
  SELECT invoicehead.ihcode,   
		...
		(select first tithcash from ticketline_invoiceline where tiilcode = invoicehead.ihcode order by tiid desc ) as cash
    FROM adresses,   
         invoicehead  
   WHERE adresses.adcode = invoicehead.ihcust and  
         date(invoicehead.ihdate) = date(:adt_day ) AND
		exists (select * from ticketline_invoiceline where tiilcode = invoicehead.ihcode)
