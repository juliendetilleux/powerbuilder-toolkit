...
```sql
  SELECT adrsactions.aacode,   
    ...
    FROM adrsactions,   
         activities,
			adresses  
   WHERE activities.accode = adrsactions.aaaction
			AND adresses.adcode = adrsactions.aaadrid
			AND adrsactions.aastatus = '3'
			AND (adrsactions.aasalorder IS NULL
				OR adrsactions.aasalline IS NULL
				OR adrsactions.aasalorder = 0 
				OR adrsactions.aasalline = 0)
			AND (adrsactions.aainvoicehead IS NULL OR adrsactions.aainvoicehead = 0 )
			AND (adrsactions.aainvoiceline IS NULL OR adrsactions.
```
