...
```sql
  SELECT itstat1.imcode,   
         ...
         clotfinit.ciqty,   
         clotfinit.cival,   
         isnull( clotfinit.ciqtywip , 0,  clotfinit.ciqtywip ) as ciqtywip
    FROM items,   
         itstat1,   
         itstat2,   
         clotfinit  
   WHERE ( items.itstat1 = itstat1.imcode ) and  
