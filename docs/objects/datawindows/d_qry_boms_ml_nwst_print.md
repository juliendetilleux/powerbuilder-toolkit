...
where methodest.endproduct in (select distinct methodest.endproduct
                                    from methodest join bomhead on bomhead.bhcode = methodest.item
                                   where isnull(bomhead.bhsubc, 'Y') = 'Y')
order by tri,
    level,
    type, 
    methodest.item 
