...
select 
	...
	convert(numeric(6,2),(select cuconv from currencies, parameters where cucode = pmcval and pmcode='SYSCURR2')) as mnt_devise,
...
and ( tickethead.thdate >= :radt_Start )   
and ( tickethead.thdate <= :radt_Stop )   
and not ((select pmcval from parameters where pmcode='SYSCURR2') <> '')
gro
```
