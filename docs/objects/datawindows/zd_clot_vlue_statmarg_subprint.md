# zd_clot_vlue_statmarg_subprint
...
```sql
  SELECT itstat1.imdesc as smname,
			...
			clotfinit.cival - 
				(select clot.cistdconf * clotfinit.ciqty
					from clotfinit as clot
					where clot.cityp = 'I' AND
						items.itcode = clot.ciitem AND
						clot.ciperiod = ihcptper ) as ilsalval
     FROM clotfinit,
		items,
		itstat1,
		itstat2
