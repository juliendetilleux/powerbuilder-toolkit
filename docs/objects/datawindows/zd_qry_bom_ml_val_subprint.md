...
```sql
  SELECT bomline.blline,   
         ...
         bh.bhxtrval,
			bh.bhcode,
			bh.bhcoeff,
			(select adcurr from adresses where adcode = '##########') as adcurr 
    FROM bomline left outer join bomhead as bh 
				ON bomline.blramcode = bh.bhcode and bh.bhtype = '0',   
         items,   
         bomhead  
